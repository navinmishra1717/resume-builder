    ...
    <div className="font-sans text-[#111827] bg-white w-full min-h-full flex text-sm leading-relaxed">
      ...
      <div className="w-[38%] bg-[#1E3A5F] text-white p-7 flex flex-col gap-5">
        ...
        {sectionVisibility.skills && skills.length > 0 && (
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-[#93C5FD] mb-2 pb-1 border-b border-[#2563EB]/40">
              Skills
            </h2>
              ...
              {skills.map((skill) => (
                ...
                >
                  ...
                  {skill.expertiseLevel && (
                    <span className="text-[#93C5FD] text-[10px] ml-2 shrink-0">
                      {skill.expertiseLevel}
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-3">
              {education.map((edu) => (
                <div key={edu.id}>
                  ...
                  {edu.location && (
                    <p className="text-[#60A5FA] text-[10px]">{edu.location}</p>
                  )}
            ...
            <div className="space-y-2">
              {certifications.map((cert) => (
                <div key={cert.id}>
                  ...
                  <p className="text-[#93C5FD] text-[10px]">
                    {cert.issuer}
                    {cert.date ? ` · ${cert.date}` : ""}
                  </p>
                </div>
      ...
      <div className="flex-1 p-7 flex flex-col gap-5">
        {sectionVisibility.experience && experience.length > 0 && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest text-[#2563EB] mb-3 pb-1 border-b-2 border-[#2563EB]">
              Experience
            </h2>
                ...
                <div key={exp.id}>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[#2563EB] font-medium text-xs">
                        {exp.company}
                  ...
                  {exp.description && (
                    <p className="mt-1 text-[#4B5563] whitespace-pre-line text-xs">
                      {exp.description}
                    </p>
                  )}
                </div>
                ...
                <div key={proj.id}>
                  <div className="flex items-center gap-2">
                    ...
                    {proj.link && (
                      <a
                        href={proj.link}
                        className="text-[#2563EB] text-[10px] underline"
                        target="_blank"
                        rel="noreferrer"
                      >
                        ↗ Link
                  ...
                  {proj.description && (
                    <p className="mt-0.5 text-[#4B5563] text-xs">
                      {proj.description}
                    </p>
                  )}
                </div>
