    ...
    <div className="font-sans text-[#111827] bg-white w-full min-h-full p-10 text-sm leading-relaxed">
      ...
      {sectionVisibility.summary && data.summary && (
        <section className="mb-5">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-[#6B7280] mb-2">
            Profile
          </h2>
          <RichContent
            html={data.summary}
            className="text-[#4B5563] text-xs [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5 [&_a]:text-[#2563EB] [&_a]:underline"
          />
        </section>
      )}
              ...
              <div key={exp.id}>
                ...
                {exp.description && (
                  <RichContent
                    ...
                    className="mt-1 text-[#4B5563] text-xs [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5 [&_a]:text-[#2563EB] [&_a]:underline"
                  />
                )}
              </div>
            ))}
              ...
              <div key={edu.id}>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {edu.gpa && (
                      <p className="text-[#6B7280] text-xs">GPA: {edu.gpa}</p>
                    )}
                  </div>
                  <p className="text-[#9CA3AF] text-xs shrink-0 ml-4">
                    {edu.startYear}
          ...
          <div className="space-y-3">
            {projects.map((proj) => (
              <div key={proj.id}>
                ...
                {proj.technologies && (
                  <p className="text-[#6B7280] text-xs">{proj.technologies}</p>
                )}
                {proj.description && (
                  <RichContent
