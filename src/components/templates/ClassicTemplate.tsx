...
export default function ClassicTemplate({ data }: Props) {
  const { personalInfo, summary, education, experience, skills, projects, certifications, hobbies, links, sectionVisibility } = data;

  const contactParts = [personalInfo.location, personalInfo.phone, personalInfo.email].filter(Boolean);
  ...
  return (
    <div className="font-serif text-[#1a1a1a] bg-white w-full min-h-full p-10 text-sm leading-relaxed" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
      {/* Header — centered */}
      <div className="text-center mb-6 resume-entry">
        <h1 className="text-4xl font-bold tracking-wide uppercase" style={{ letterSpacing: "0.05em" }}>
          {personalInfo.name || "Your Name"}
        </h1>
        <h2 className="text-2xl" style={{ letterSpacing: "0.05em" }}>
          {personalInfo.name && experience.length > 0 && experience[0].role ? ` ${experience[0].role}` : ""}
        </h2>
        {contactParts.length > 0 && <p className="text-sm text-[#444] mt-1">{contactParts.join("  ·  ")}</p>}
        {sectionVisibility.links && links && links.length > 0 && (
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-1">
            {links.filter((l) => l.name || l.link).map((l) => (
              <a key={l.id} href={l.link} target="_blank" rel="noreferrer" className="text-[11px] text-[#1a1a1a] underline">
                {l.name || l.link}
              </a>
            ))}
          </div>
        ...
        <div className="resume-section">
          <div className="flex gap-0">
            ...
            <div className="flex-1 pt-3 pb-3 pl-6">
              <RichContent html={summary} className="text-sm text-[#333] [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5" />
            </div>
          </div>
            ...
            <div className="flex-1 pt-3 pb-3 pl-6 space-y-4">
              {experience.map((exp) => (
                <div key={exp.id} className="resume-entry">
                  ...
                  {exp.description && (
                    <RichContent html={exp.description} className="mt-1 text-sm text-[#333] [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5" />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="flex-1 pt-3 pb-3 pl-6 space-y-4">
              {education.map((edu) => (
                <div key={edu.id} className="resume-entry">
                  ...
                  {edu.description && (
                    <RichContent html={edu.description} className="mt-1 text-sm text-[#333] [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5" />
                  )}
                </div>
              ))}
            </div>
              ...
              {projects.map((proj) => (
                <div key={proj.id} className="resume-entry">
                  <div className="flex items-baseline gap-2">
                    <h3 className="font-bold text-sm text-[#1a1a1a]">{proj.title || "Project"}</h3>
                    {proj.link && (
                      <a href={proj.link} className="text-sm underline text-[#1a1a1a]" target="_blank" rel="noreferrer">Link</a>
                    )}
                  </div>
                  {proj.technologies && <p className="text-xs text-[#666] leading-tight">{proj.technologies}</p>}
                  {proj.description && (
                    <RichContent html={proj.description} className="mt-0.5 text-sm text-[#444] [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4" />
                  )}
                </div>
              ))}
            </div>
